...
public class Node implements REST {
    ...
    public void get(HttpServletRequest req, HttpServletResponse res) {
        String pathInfo = req.getPathInfo(); // e.g., "/status" or "/config"
        ...
        try {
            if ("/status".equals(pathInfo)) {
                ...
                OutputProcessor.errorResponse(res, HttpServletResponse.SC_NOT_FOUND, "Not Found", "Resource not found for GET method.", req.getRequestURI());
            }
    ...
    public void post(HttpServletRequest req, HttpServletResponse res) {
        String pathInfo = req.getPathInfo(); // e.g., "/config/pki/csr" or "/config/pki/certificate"
        ...
        try {
            ...
            if ("/config/pki/csr".equals(pathInfo)) {
                ...
                if (commonName.isEmpty() || organization.isEmpty()) {
                    OutputProcessor.errorResponse(res, HttpServletResponse.SC_BAD_REQUEST, "Bad Request", "Missing required fields (common_name, organization).", req.getRequestURI());
                    return;
                }
                output = generateCsrAndStorePrivateKey(commonName, organization);
                OutputProcessor.send(res, HttpServletResponse.SC_OK, output);

            } else if ("/config/pki/certificate".equals(pathInfo)) {
                ...
                if (certificatePem.isEmpty() || privateKeyPem.isEmpty()) {
                    OutputProcessor.errorResponse(res, HttpServletResponse.SC_BAD_REQUEST, "Bad Request", "Missing required fields (certificate_pem, private_key_pem).", req.getRequestURI());
                    return;
                }
                ...
                OutputProcessor.errorResponse(res, HttpServletResponse.SC_NOT_FOUND, "Not Found", "Resource not found for POST method.", req.getRequestURI());
            }
    ...
    public void put(HttpServletRequest req, HttpServletResponse res) {
        String pathInfo = req.getPathInfo(); // Should be "/config"
        JSONObject input = null;
        JSONObject output = null;

        if (!"/config".equals(pathInfo)) {
            OutputProcessor.errorResponse(res, HttpServletResponse.SC_NOT_FOUND, "Not Found", "Resource not found for PUT method.", req.getRequestURI());
            return;
        }
        ...
        try {
            ...
            // Validate input fields for update (e.g., node_id, fqdn, network_port, storage paths, logging level)
            ...
            if (nodeId.isEmpty() || fqdn.isEmpty() || networkPort == -1 || storageActivePath.isEmpty() || storageArchivePath.isEmpty() || loggingLevel.isEmpty()) {
                OutputProcessor.errorResponse(res, HttpServletResponse.SC_BAD_REQUEST, "Bad Request", "Missing or invalid required configuration fields.", req.getRequestURI());
                return;
            }
    ...
    public void delete(HttpServletRequest req, HttpServletResponse res) {
        OutputProcessor.errorResponse(res, HttpServletResponse.SC_METHOD_NOT_ALLOWED, "Method Not Allowed", "DELETE method not supported for node management.", req.getRequestURI());
    }
    ...
    public boolean validate(String method, HttpServletRequest req, HttpServletResponse res) {
        ...
        try {
            ...
            // Basic input validation for POST/PUT methods (e.g., content type)
            if ("POST".equalsIgnoreCase(method) || "PUT".equalsIgnoreCase(method)) {
                return InputProcessor.validate(req, res); // This validates content-type and basic body parsing
            }

            return true; // Valid for GET requests
        } catch (Exception e) {
            e.printStackTrace();
            OutputProcessor.errorResponse(res, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Internal Server Error", "Token validation failed: " + e.getMessage(), req.getRequestURI());
            return false;
        }
    }
    ...
    private JSONObject importCertificateAndActivate(String certificatePem, String privateKeyPem) throws Exception {
        ...
        try {
            ...
            saveCertificateToDb(NODE_CONFIG_SINGLETON_ID, certificatePem, privateKeyPem, true, Timestamp.valueOf(LocalDateTime.now()), expiresAt);

            // Optionally, update the node_config table's FQDN if it changed based on the new cert
            // This assumes the node's FQDN is derived from its active certificate
            ...
            pool.cleanup(null, pstmtUpdateOld, null);
            pool.cleanup(null, pstmtInsertNew, conn); // conn is cleaned up last
        }
    }
    ...
    private void saveCertificateToDb(UUID nodeConfigId, String certificatePem, String privateKeyPem, boolean isActive, Timestamp issuedAt, Timestamp expiresAt) throws SQLException {
        ...
        // If you want to update an existing entry (e.g., the 'pending' private key entry), you'd use an UPDATE statement
        // For simplicity here, we're inserting a new record each time a key/cert pair is generated/imported.
        // A more robust system would manage a single active cert per node_config.
